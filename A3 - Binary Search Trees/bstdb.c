...
// fallowing task 1, we first need a struct for nodes
// we got to store name and wordcount + a uniqe id for books
// some examples i looked at:https://stackoverflow.com/questions/18536778/alphabetically-sorted-binary-search-tree-in-c
////And aslo:https://stackoverflow.com/questions/2130416/what-are-the-applications-of-binary-trees
...
typedef struct Book_node
{
	...
	struct Book_node *right; // so far im fallowing what i did in task 1 (similer to bst.h)

}Book_node;

// now here we create a node just like we did with task 1 main file
//we'll have more variables in the ()

struct Book_node* bnoddy(char author, int word_count, char name)
{
  Book_node* k = (Book_node*)malloc(sizeof(struct Book_node));
  k -> name = name;
  k -> word_count = word_count;
  k -> author = author;
...
bstdb_add ( char *name, int word_count, char *author ) {
	...
	if(*name == NULL)
	{
		retrun Book_node(name);
		retrun Book_node(word_count);
		retrun Book_node(author);
	}
	else if(name > name -> data || word_count > word_count -> data || author > author -> data )
	{
		bstdb_add(name);
		bstdb_add(word_count);
		bstdb_add(author);
	}
...
bstdb_get_word_count ( int doc_id ) {
	// This is a search function. It should traverse the binary search tree (use task 1 for that)
    ...
    else if(doc_id ->data == data)
    {
        return 1;
    }
    else if(doc_id > root->data ) 
    {
        return bstdb_add(doc_id->left,data);
    }
    else
    {
        return bstdb_add(doc_id->right,data);
    }
...
bstdb_get_name ( int doc_id ) {
    ...
    else if(name >data == data)
    {
        return 1;
    }
    else if(name > root->data ) 
    {
        return bstdb_add (name->left,data);
    }
    else
    {
        return bstdb_add(name->right,data);
    }
